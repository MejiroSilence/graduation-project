...
class qscan(nn.Module):
    def __init__(self, args):
        ...
        self.encoder=torch.nn.TransformerEncoder(self.encoder_layer, args.attentionLayer)
    ...
    def forward(self,batch,t=None):
        ...
        inputs=self.embed(inputs)* math.sqrt(self.args.embedSize)
...
class qpair_fix(nn.Module):
    def __init__(self, stateDim,hiddenSize,agentNum,actionNum):
        super(qpair_fix, self).__init__()
        self.agentNum=agentNum
        self.actionNum=actionNum
        self.hyper_w1=nn.Sequential(
            nn.Linear(stateDim+2*agentNum+2*actionNum, hiddenSize),
            nn.ReLU(inplace=True),
            nn.Linear(hiddenSize, 2*32),
            #nn.Softmax(dim=-1)
        )
    ...
    def forward(self,batch,qvals,t=None):
        qvals=qvals.view((batch.batchSize,batch.max_seq_length if t is None else 1,1,-1))
        inputs=self.buildInput(batch,t)
        w1_outs=self.hyper_w1(inputs).reshape((batch.batchSize,-1,self.combSize,2,32))#b*t*comb*2*32
        ts = slice(None) if t is None else slice(t, t+1)
        state=batch.data.states[:, ts].view((batch.batchSize,batch.max_seq_length if t is None else 1,-1))
        b1=self.hyper_b1(state).view((batch.batchSize,-1,1,32))#b*t*1*32
        w1=torch.zeros((batch.batchSize,batch.max_seq_length if t is None else 1,self.agentNum,32),device=device)
        for i in range(self.agentNum):
            for j in range(self.agentNum):
                if i==j:
                    continue
                if i<j:
                    w1[:,:,i]+=w1_outs[:,:,int((2*self.agentNum-1-i)*i/2+j-i-1),0].abs()
                else:
                    w1[:,:,i]+=w1_outs[:,:,int((2*self.agentNum-1-j)*j/2+i-j-1),1].abs()
        hidden = F.elu(torch.matmul(qvals, w1) + b1)
        w2=self.hyper_w2(state).reshape((batch.batchSize,-1,32,1)).abs()#b*t*32*1
        b2=self.hyper_b2(state).view((batch.batchSize,-1,1,1))#b*t*1*1
        y = torch.matmul(hidden, w2) + b2 

        return y.reshape((batch.batchSize,batch.max_seq_length if t is None else 1,-1))
