...
class Qnet(nn.Module):
    ...
    def _build_inputs(self, batch, t=None):
        ...
        inputs = []
        # state
        inputs.append(batch.data.states[:, ts].unsqueeze(2).repeat(1, 1, self.n_agents, 1))

        # observation
        inputs.append(batch.data.obs[:, ts])

        # actions (masked out by agent)
        actions = batch.data.actionsOnehot[:, ts].view(bs, max_t, 1, -1).repeat(1, 1, self.n_agents, 1)
